...
class SmartoTourAssets {
    ...
    getNavButtons() {
        return `
            <div class="s3d-smarto-tours__navbar">
                ${this.data.fields.map(el => `<button class="ButtonIconLeft" type="button" data-aframe-src="#${el.title}"><span>${el.image_title}</span></button>`).join('')}
            </div>
        `;
    }
    ...
    getFlatGenplan(flat_plan_data = [], tourData, flat_level_photos = {}) {
        ...
        <div class="flat-plan">
            <img src="${url}" alt="">
            ${flat_plan_data.map((data, index) => {
                const { left, top, scene, id, angle } = data;                
                const sceneForThisLink = Object.entries(tourData.fields).find(el1 => el1[0] == scene) ? Object.entries(tourData.fields).find(el1 => el1[0] == scene)[1] : null;
    ...
    getLayout({ uniqueByLevel }) {
        return `
            <div class="s3d-smarto-tours__menu">
                <div class="s3d-smarto-tours__nav-button s3d-smarto-tours__floorplan-toggle" title="${this.i18n.t('SmartoTours.floor_show_or_hide')}">
                ...
                <div class="s3d-smarto-tours__nav-button" data-smarto-tours-nav title="${this.i18n.t('SmartoTours.menu')}">
            ...
            <div class="flatplan-wrapper">
                <div class="flatplan-wrapper__nav">
                    ${uniqueByLevel.map((levelData, index) => {
                        const title = get(this.customFloorTitles, levelData.property_level, `${this.i18n.t('SmartoTours.floor')} ${levelData.property_level}`);
                        const order = get(this.customNavFloorsOrders, index, null);
                        return `<div ${order ? `style="order: ${order}"` : ''} class="flat-plan-level-button" data-flatplan-floor-button="${levelData.property_level}" data-aframe-src="#${levelData.title}">
                                ${title}
                            </div>
                        `;
                    }).join('')}
...
export default class SmartoTours extends SmartoTourAssets {
    constructor(config) {
        ...
        this.defaultScene = config.defaultSceneIndex && this.data.fields[config.defaultSceneIndex] ? this.data.fields[config.defaultSceneIndex] : this.data.fields[0];
        this.LOADED_SCENES = [];
    ...
    loadDefaultScene() {
        ...
        this.data.fields.forEach((el, index) => {
            ...
            img.onload = () => {
                ...
                this.loadRestScenes();
                setTimeout(() => {
                    document.querySelector('[data-custom-loading-screen]').remove();
                }, 1500);
            };
    ...
    initCustomFullscreenHandler() {
        ...
        this.$container.querySelector(`[${this.OPTIONS_ATTRIBUTE}]`).innerHTML = `
            <div class="s3d-smarto-tours__nav-button s3d-smarto-tours__nav-button--fullscreen" title="${this.i18n.t('SmartoTours.fullscreen')}" data-smarto-tours-fullscreen>
    ...
    handleFlatPlanPins() {
        ...
        flatPlanPins.forEach(pin => {
            ...
            gsap.to($activeFlatPlanPin.querySelector('svg'), { rotate: panoramaYawMapRange + angleOffsetForCurrentPin - 180, duration: 0.1 });
        });
    }
    ...
    handleFlatPlanImage(sceneId) {
        ...
        document.querySelectorAll('[data-preview-pin-level]').forEach(pin => {
        ...
        document.querySelectorAll('[data-flatplan-floor-button]').forEach(btn => {
            btn.classList.remove('active');
    ...
    getScene(children = '') {
        return `
            ...
            >
                ...
                <a-assets>
                    <img crossorigin="anonymous" id="${this.HOTSPOT_ASSETS_ID}" src="/wp-content/themes/3d/assets/s3d/images/smarto-tours-pin.svg"/>

                </a-assets>
                <a-entity laser-controls="hand: right" raycaster="objects: .collidable; far: ${this.SPHERE_RADIUS * 1.5}" laser-events></a-entity>

                <a-sphere
                    ${Object.entries(this.getSphereAttributes()).map(([key, value]) => `${key}="${value}"`).join(',\n')}
                >
                </a-sphere>
    ...
    renderSceneHotSpots() {
        ...
        if (aScene) {
            ...
            document.querySelectorAll('[data-hotspot-preview-image]').forEach(img => img.remove());
        }
        this.data.fields.forEach(scene => {
            ...
            scene.image_pin_data.forEach(hotspot => {
                ...
                setTimeout(() => {
                    ...
                    hotspotEl.insertAdjacentHTML('afterend', this.createCurvedImageForHotspotPreview(hotspotEl));
                    setTimeout(() => {
    ...
    renderCssHotspots() {
        ...
        this.data.fields.filter(el => el.title === this.currentSceneId).forEach(scene => {
            scene.image_pin_data.forEach(hotspot => {
                ...
                cssContainer.insertAdjacentHTML('beforeend', `
                <div class="hotspot-label hidden" id="${safeId}" ${isTouchDevice ? '' : changeSceneAttribute}>
                ...
                hotspotEl.setAttribute('position', this.pannellumToAframe(
                hotspot.pitch + 20,
                hotspot.yaw - 90,
    ...
    getHotspotAttributes(hotspot, scene) {
        return {
            ...
            clickable: '',
            'data-scene': (hotspot.sceneId !== undefined && this.data.fields[hotspot.sceneId]) ? `#${this.data.fields[hotspot.sceneId].title}` : '',
            ...
            position: this.pannellumToAframe(hotspot.pitch + 20, hotspot.yaw - 90, this.SPHERE_RADIUS*0.9),
            visible: true,
        };
    }
    getSphereAttributes() {
    ...
    getTextAttributes(hotspot) {
        return {
            ...
            position: this.pannellumToAframe(hotspot.pitch + 20, hotspot.yaw - 90, this.SPHERE_RADIUS*0.8),
            'look-at': '#cam'
        }
    }
...
export const SMARTO_TOURS_CONTAINER_SELECTOR = '[data-3d_tour_v2_container]';
