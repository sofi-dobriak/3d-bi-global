...
import { destroyAFrameScene } from './SmartoToursV3/helpers_SmartoToursV3';
...
class SmartoToursV3 extends SmartoTourV3Assets {  
    constructor(config) {
        ...
        this.defaultScene = config.defaultSceneIndex && this.data[config.defaultSceneIndex] ? this.data[config.defaultSceneIndex] : this.data[0];
    ...
    init() {
        ...
        this.$container.addEventListener('pointerdown', (evt) => {
            ...
            this.startScrollTop = this.scrollableElement === window ? window.scrollY : this.scrollableElement.scrollTop;
        });
        ...
        this.$container.querySelector('.s3d-smarto-tours__floorplan-toggle').addEventListener('click', () => {
            this.navBar.next({ 
        ...
        this.$container.querySelector('.s3d-smarto-tours__textures-toggle').addEventListener('click', () => {
            this.navBar.next({
                ...Object.entries(this.navBar.value).reduce((acc, [key, value]) => {
                    acc[key] = false;
                    return acc;
        ...
        this.scene$.subscribe(({ sceneId, data }) => {
            ...
            this.$container.querySelector('[data-textures-buttons]').innerHTML = `
                ${Array.isArray(data.tour_v2_premises_textures) ? 
                    data.tour_v2_premises_textures.map(textureGroup => this.getTextureGroupForNavMenu(textureGroup, data)).join('') : 
                    ''
                }
            `;
            ...
            if (!currentScene.loadedInnerAssets && Array.isArray(currentScene.tour_v2_premises_textures)) {
            ...
            if (Array.isArray(currentScene.tour_v2_premises_textures)) {
                currentScene.tour_v2_premises_textures.forEach(textureGroup => {
                    this.$scene.insertAdjacentHTML('beforeend', this.getSphereForPremiseInnerTexture(textureGroup.tour_v2_single_texture_group_id))
                });
            }
        ...
        this.$container.addEventListener('click', (evt) => {
            ...
            if (this.is_in_vr) {
                if (textureImageId != 'default') textureSphere.setAttribute('material', `src: #${textureImageId}`);
            ...
            if (textureImageId != 'default' && textureSphere.getAttribute('visible') === false && !this.is_in_vr) {
            ...
            } else if (textureImageId == 'default' && textureSphere.getAttribute('visible') === true && !this.is_in_vr) {
                gsap.timeline()
                    .fromTo(textureSphere, {
                        attr: {
                            opacity: 1
    ...
    initCustomFullscreenHandler() {
        ...
        this.$container.querySelector('a-scene').setAttribute('xr-mode-ui', `enabled: ${AFRAME.utils.device.checkHeadsetConnected() ? 'true' : 'false'}`);
        this.$container.addEventListener('click', (evt) => {
        ...
        document.addEventListener('fullscreenchange', () => {
            this.$container.querySelector('[data-smarto-tours-fullscreen]').classList.toggle('active', !!document.fullscreenElement);
        });

        this.$container.querySelector(`[${this.OPTIONS_ATTRIBUTE}]`).innerHTML = this.getFullscreenButton();
    }
    ...
    handleFlatPlanPins() {
        ...
        flatPlanPins.forEach(pin => {
            ...
            const angleOfActiveFlatPlanPin = $activeFlatPlanPin.dataset.angle ? +$activeFlatPlanPin.dataset.angle : 0;
            const angleOffsetForCurrentPin = gsap.utils.mapRange(-180, 180, 0, 360, angleOfActiveFlatPlanPin);
            ...
            gsap.to($activeFlatPlanPin.querySelector('svg'), { x: 0, y: 0, xPercent: -50, yPercent: -50, rotate: panoramaYawMapRange + angleOffsetForCurrentPin - 180, duration: 0 });
        });
    }
    ...
    removeVrModeHoverTextureGroupImages() {
        this.$container.querySelectorAll('[data-vr-texture-group-hover]').forEach(group => group.remove());
    }
    ...
    renderVrModeHotspots(aScene) {
        ...
        document.querySelectorAll('.collidable').forEach(el => {
            ...
            el.addEventListener('mouseenter', () => {
                ...
                document.querySelectorAll(`[data-hotspot-preview-image="${el.getAttribute('data-scene')}"]`).forEach(img => {
                    img.setAttribute('visible', true);
                });
            });
    ...
    renderVrModeHoverTextureGroupImages(textureGroupPin) {
        ...
        const dir = new THREE.Vector3();
        dir.subVectors(camera.object3D.position, textureGroupPin.object3D.position).normalize();
        positionCloserToCamera.copy(textureGroupPin.object3D.position).addScaledVector(dir, this.SPHERE_RADIUS * 0.1);

        const currentScene = this.data.find(scene => {
            if (!Array.isArray(scene.tour_v2_premises_textures)) return false;
            const textures = scene.tour_v2_premises_textures.find(tex => tex.tour_v2_single_texture_group_id === textureGroupId);
            return textures;
        });

        const textureGroup = currentScene.tour_v2_premises_textures.find(tex => tex.tour_v2_single_texture_group_id === textureGroupId);

        const textureGroupItemsPositions = this.vrTextureGroupItemsCoords;
        
        const vrTextureGroupHover = `
            <a-entity
                position="${positionCloserToCamera.x} ${positionCloserToCamera.y} ${positionCloserToCamera.z}"
                rotation="0 0 0"
            ...
            >
                ${this.getTextureGroupHoverImageItem({ 
                    ...
                    src: textureGroup.tour_v2_single_texture_group_default_preview ? textureGroup.tour_v2_single_texture_group_default_preview : '' 
                })}
                ${Array.isArray(textureGroup.tour_v2_single_texture_group_images) ? textureGroup.tour_v2_single_texture_group_images.map((texGroup, index) => `
                    ${this.getTextureGroupHoverImageItem({ 
                        ...
                        src: texGroup.tour_v2_single_texture_group_images_preview ? texGroup.tour_v2_single_texture_group_images_preview : '' 
                    })}}
    ...
    renderCssHotspots() {
        ...
        this.data.filter(el => el.tour_v2_premise_id === this.currentSceneId).forEach(scene => {
            ...
            scene.tour_v2_premise_hotspots.forEach((hotspot, index) => {
                const imgSrc = this.data.find(el => el.tour_v2_premise_id === hotspot.source_scene_id) || { tour_v2_premise_img: { url: '' } };
                const title = this.data.find(el => el.tour_v2_premise_id === hotspot.source_scene_id) || {};
                const safeId = uniqueId();
                const changeSceneAttribute = `data-aframe-src="#${hotspot.hotspot_id}"`;
                const isTouchDevice = ('ontouchstart' in window || navigator.maxTouchPoints > 0 || navigator.msMaxTouchPoints > 0);

                cssContainer.insertAdjacentHTML('beforeend', hotspotTemplatesMap[hotspot.type]({
                    hotspot,
                    safeId,
                    title,
                    imgSrc,
    ...
    changeScene(sceneId, target) {
        ...
        const isSphereLoaded = this.LOADED_SCENES.some(el => el.tour_v2_premise_id === sceneId.replace('#', ''));
        ...
        const tl = gsap.timeline({
            paused: true
        }).fromTo(material.uniforms.progress,
            { value: 0 },
            {
                value: 1,
                duration: 1,
        ...
        this.scene$.next({ sceneId: this.currentSceneId, data: this.data.find(el => `${el.tour_v2_premise_id}` === this.currentSceneId) });
    ...
    createCurvedImageForHotspotPreview(hotspot) {
        ...
        const currentScene = this.data.find(el => el.tour_v2_premise_id === hotspot.source_scene_id) || {};
        
        const text = currentScene.tour_v2_premise_title;
        const textLength = text.length;
        
        const YOffset = y + this.SPHERE_RADIUS / 10;
        const textAndBgYPosition =
            YOffset - (this.SPHERE_RADIUS / 5 / 2) +
            (this.SPHERE_RADIUS / 5) +
            0.1;
        return this.getLayoutOfCurvedImageForHotspotPreview({ hotspot, x, YOffset, z, textAndBgYPosition, textLength, currentScene })
    }

    getHotspotAttributes(hotspot, scene) {

        const source_scene = this.data.find(el => el.tour_v2_premise_id == hotspot.source_scene_id) || {};

        const src = hotspot.type == 'scene' ? `#${this.HOTSPOT_ASSETS_ID}` : `#${this.HOTSPOT_TEXTURE_ASSETS_ID}`;

        if (hotspot.type == 'texture') {
            return {
                class: 'collidable-texture-group',
                clickable: '', 
                geometry: `primitive: box; width: ${this.SPHERE_RADIUS / 15}; height: ${this.SPHERE_RADIUS / 15}; depth: ${this.SPHERE_RADIUS / 30}; segmentsWidth: 16`,
                scale: '1 1 1',
        ...
        return {
            ...
            clickable: '',
            'data-scene': (hotspot.source_scene_id !== undefined && source_scene) ? `#${source_scene.tour_v2_premise_id}` : '',
            geometry: `primitive: box; width: ${this.SPHERE_RADIUS / 15}; height: ${this.SPHERE_RADIUS / 10}; depth: ${this.SPHERE_RADIUS / 30}; segmentsWidth: 16`,
            scale: '1 1 1',
