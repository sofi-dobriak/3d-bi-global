...
export default class SmartoTourV3Assets {
    ...
    getNavButtons() {
        return `
            <div class="s3d-smarto-tours__navbar">
                ${this.data.map(el => `<button class="ButtonIconLeft" type="button" data-aframe-src="#${el.tour_v2_premise_id}"><span>${el.tour_v2_premise_title}</span></button>`).join('')}
            </div>
        `;
    }
        ...
        return `
            <div class="flat-plan" data-smarto-tours-flatplan>
                <img src="${url}" alt="" data-smarto-tours-flatplan-image/>
                    ${flat_plan.map((data, index) => {
                    const { xPercent, yPercent, scene, id, currentSceneId, pointOfView } = data;
                    const sceneForThisLink = tourData
                        .find(tour => tour.tour_v2_premise_id == currentSceneId);
    ...
    getLayout({ uniqueByLevel }) {
        return `
            <div class="s3d-smarto-tours__menu">
                <div class="s3d-smarto-tours__nav-button s3d-smarto-tours__textures-toggle" title="Варіанти">
            ...
            <div class="flatplan-wrapper">
                <div class="flatplan-wrapper__nav">
                    ${uniqueByLevel.map((levelData, index) => {
                        const title = get(this.customFloorTitles, levelData.tour_v2_premise_level, `${this.i18n.t('SmartoTours.floor')} ${levelData.tour_v2_premise_level}`);
                        const order = get(this.customNavFloorsOrders, index, null);
                        return `<div ${order ? `style="order: ${order}"` : ''} class="flat-plan-level-button" data-flatplan-floor-button="${levelData.tour_v2_premise_level}" data-aframe-src="#${levelData.tour_v2_premise_id}">
                                ${title}
                            </div>
                        `;
        }).join('')}
                </div>
                ${this.getFlatGenplan(this.flat_plan_data, this.data, this.flat_level_photos, this.flat_plan)}
            </div>
    ...
    getSceneCssHotspot({
        ...
        return `
            <div data-scene-id="${hotspot.source_scene_id}" data-hotspot-index="${index}" class="hotspot-label hidden" id="${safeId}" ${isTouchDevice ? '' : changeSceneAttribute}>
    ...
    getTextureGroupCss({
        ...
        const currentScene = this.data.find(el => {
            ...
            const textures = el.tour_v2_premises_textures.find(tex => tex.tour_v2_single_texture_group_id === hotspot.hotspot_id.replace('texture-', ''));
            return textures;
        });
        const currentTextureGroup = currentScene ? currentScene.tour_v2_premises_textures.find(tex => tex.tour_v2_single_texture_group_id === hotspot.hotspot_id.replace('texture-', '')) : null;
        ...
        return `
            <div data-scene-id="${hotspot.source_scene_id}" data-hotspot-index="${index}"  class="menu-container" id="${safeId}">
                ...
                <div 
                    ...
                    style="background: url(${currentTextureGroup.tour_v2_single_texture_group_default_preview}) no-repeat center center; background-size: cover;"
                >
                    <span class="option-label">
                        ${currentTextureGroup.tour_v2_single_texture_group_title ? currentTextureGroup.tour_v2_single_texture_group_title : 'Default'}
                    </span>
                
                </div> 
                ${currentTextureGroup && Array.isArray(currentTextureGroup.tour_v2_single_texture_group_images) ? currentTextureGroup.tour_v2_single_texture_group_images.map((texture, index) => `
                    <div 
                        data-texture-image="${texture.tour_v2_single_texture_group_images_id}" 
                        data-texture-group-id="${currentTextureGroup.tour_v2_single_texture_group_id}"
                        class="option-item material-${index + 2}"
                        style="background: url(${texture.tour_v2_single_texture_group_images_preview}) no-repeat center center; background-size: cover;"
                    >
                        <span class="option-label">
    ...
    getScene(children = '') {
        return `
            ...
            >
                ...
                <a-assets>
                    <img crossorigin="anonymous" id="${this.HOTSPOT_ASSETS_ID}" src="${this.themeFolder}assets/s3d/images/smarto-tours-v3/smarto-tours-pin.svg"/>
                    <img crossorigin="anonymous" id="${this.HOTSPOT_TEXTURE_ASSETS_ID}" src="${this.themeFolder}assets/s3d/images/smarto-tours-v3/smarto-text ure-group-pin-preview.svg"/>

                </a-assets>
                <a-entity laser-controls="hand: right" raycaster="objects: .collidable, .collidable-texture-group; far: ${this.SPHERE_RADIUS * 1.5}" laser-events></a-entity>
                ...
                <ul>
                    <li>
                        <button 
                            ...
                            type="button"
                        >
                            <span>${textureGroup.tour_v2_single_texture_group_title ? textureGroup.tour_v2_single_texture_group_title : 'Default'}</span>
                    ...
                    ${Array.isArray(textureGroup.tour_v2_single_texture_group_images) ? textureGroup.tour_v2_single_texture_group_images.map(texture => `
                        <li>
                            <button 
                                class="s3d-smarto-tours__texture-button"
                                data-texture-image="${texture.tour_v2_single_texture_group_images_id}" 
    ...
    getLayoutOfCurvedImageForHotspotPreview({ hotspot, x, YOffset, z, textAndBgYPosition, textLength, currentScene }) {
        return `
            <a-image 
