...
export default function MobileFlybyController(i18n) {
  ...
  return `
    <div class="MobileFlybyController">
      ...
      ${MenuMobileButton('js-s3d-nav__btn', 'data-type="plannings"', undefined, 'Card', 'brand')}
      ${MenuMobileButton(
        '',
      ...
      <div class="js-price-link-container"></div>
      <!--<button class="ButtonIconLeft ButtonIconLeft--secondary ButtonIconLeft--centered" data-open-form="" ${showElementsOnPageAttribute(
        ['flat'],
      )}>
...
function MobileFloorFunctionsMenu(i18n) {
  ...
  return `
    <div class="MobileFunctionsMenu" data-mobile-floor-functions-menu >
      ...
      <div class="text-style-3-d-fonts-1920-h-2-regular MobileFunctionsMenu__title">
        ${i18n.t('ctr.menu.btn.chooseOfFloor')}
      </div>
...
function MobileFunctionsMenu(i18n) {
  ...
  const flybys = Object.entries(config).filter(
    ([key, value]) => key === 'flyby' || key === 'genplan',
  );

  let flybysWithDarkTheme = flybys.map(flyby => {
    if (flyby[0] === 'genplan' || flyby[1].imageUrlDark) {
      return 'genplan';
    }
    return Object.entries(flyby[1]).map(([number, sides]) => {
      return Object.entries(sides).map(([side, flybyData]) => {
        return flybyData.imageUrlDark ? `flyby_${number}_${side}` : null;
      });
    });
  });

  flybysWithDarkTheme = flattenDeep(flybysWithDarkTheme).filter(Boolean);
  ...
  return `
    <div class="MobileFunctionsMenu" data-mobile-functions-menu >
      ...
      <div class="MobileFunctionsMenu__row">
        ${document.documentElement.classList.contains('desktop') ? '' : $highlightSvgElements(i18n)}
        <!--<div class="MobileFunctionsMenu__group" ${showElementsOnPageAttribute(
          flybysWithDarkTheme,
        )}>
          ${IconButton('', 'data-mobile-theme-switcher="light"', 'Sun')}
          ${IconButton('', 'data-mobile-theme-switcher="dark"', 'Moon')}
        </div>-->
