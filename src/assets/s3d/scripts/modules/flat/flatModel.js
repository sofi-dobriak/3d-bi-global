...
} from '../../../../s3d2/scripts/modules/AudioAssistant/SmartoTours';
import { isJson } from '../helpers/helpers';
import SmartoToursV3, {
  SMARTO_TOURS_V3_CONTAINER_SELECTOR,
} from '../../../../s3d2/scripts/modules/AudioAssistant/SmartoToursV3';
...
class FlatModel extends EventEmitter {
  ...
  async update(id) {
    ...
    //   );
    // }
    // document.querySelector('[data-header-flat-plan-group][data-type="flat"]').textContent = this.getFlat(this.activeFlat).number;
  ...
  setPlaneInPage(flatId) {
    ...
    const html = htmlMapping[this.viewType](
      ...
      this.constructionProgressDataList,
      this.show_prices,
      this.getFlat,
  ...
  toggleAnimationCircle(dataId) {
    document
      .querySelector(`.SvgFloorPolygonTooltip__title[data-id="${dataId}"]`)
  ...
  initSmartoTours() {
    ...
    const selector = SMARTO_TOURS_CONTAINER_SELECTOR;
  ...
  initSmartoToursV3() {
    ...
    if (!tourDataUrl) return;
    const selector = SMARTO_TOURS_V3_CONTAINER_SELECTOR;
